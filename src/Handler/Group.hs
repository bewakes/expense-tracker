...
getGroupR = loginRedirectOr $ \(UserInfo uid _ _) -> do
...
getGroupNewR = loginRedirectOr $ \(UserInfo uid _ _)-> do
        (widget, enctype) <- generateFormPost $ renderBootstrap3 BootstrapBasicForm $ groupForm uid
        defaultLayout $(widgetFile "groups/new")

postGroupNewR :: Handler Html
postGroupNewR = loginRedirectOr $ \(UserInfo uid _ _) -> do
    ((res, widget), enctype) <- runFormPost $ renderBootstrap3 BootstrapBasicForm $ groupForm uid
    time <- liftIO getCurrentTime
    case res of
      FormSuccess grp -> do
                groupId <- runDB $ insert grp
                let usrgrp = UsersGroups {
                          usersGroupsUserId     = uid
...
getGroupNewMemberR gid = loginRedirectOr $ \(UserInfo uid _ _)-> do
